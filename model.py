...
train_path = r"C:\Users\Cole\Desktop\pokemon scanner latest with big dataset\dataset_for_model\train"
...
metrics_folder = r"C:\Users\Cole\Desktop\pokemon scanner latest with big dataset\dataset_for_model\metrics_for_set_modelel"
...
def reverse_preprocess_input(x):
    ...
    """
    x = (x + 1.0) * 127.5  # From [-1, 1] to [0, 255]
    x = np.clip(x, 0, 255)
    return x.astype("uint8")

def preview_augmentations(image_paths_with_ids, datagen, num_samples=5, augmentations_per_image=3):
    """
    Preview a few cards with the applied augmentations (with MobileNetV2 preprocessing reversed for display).
    """
    selected_samples = random.sample(image_paths_with_ids, min(num_samples, len(image_paths_with_ids)))

    for img_path, card_id in selected_samples:
        try:
        ...
        plt.figure(figsize=(augmentations_per_image * 2.5, 3))
        for i in range(augmentations_per_image):
            aug_img = next(aug_iter)[0]
            aug_img = reverse_preprocess_input(aug_img)
            plt.subplot(1, augmentations_per_image, i + 1)
            plt.imshow(aug_img)
            plt.axis('off')
            plt.title(f"{card_id}")
        plt.suptitle(f"Augmented views of {card_id}", fontsize=14)
        plt.tight_layout()
        plt.show()
...
def save_metrics_plot(history, set_name):
    ...
    metrics_folder = r"C:\Users\Cole\Desktop\pokemon scanner latest with big dataset\card_id_metrics"
...
def train_card_id_model_loocv(set_name, image_paths_with_ids, rarities, unique_rarities, metrics_folder):
    ...
    # Augmentation pipeline with tilting, camera angle, distance, and blur
    datagen = ImageDataGenerator(
        rotation_range=10,          # Slightly increased rotation
        width_shift_range=0.2,      # Randomly shift images horizontally
        height_shift_range=0.2,     # Randomly shift images vertically
        shear_range=0.2,            # Apply minor shearing
        zoom_range=[0.9, 1.1],       # Slightly adjusted zoom range
        horizontal_flip=False,      # No horizontal flipping
        fill_mode='constant',        # Fill with reflection
        cval=0,
        brightness_range=[0.8, 1.2], # Random brightness adjustments
    )

    #preview_augmentations(image_paths_with_ids, datagen, num_samples=3, augmentations_per_image=4)
...
csv_folder = r'C:\Users\Cole\Desktop\pokemon scanner latest with big dataset\151_data'
metrics_folder = r"C:\Users\Cole\Desktop\pokemon scanner latest with big dataset\card_id_metrics"
